...
function scoreStyle(score: number) {
  if (score === 100) return { bg: "rgba(201,168,76,0.18)",  text: "#c9a84c", label: "Exact", border: "rgba(201,168,76,0.35)" };
  if (score === 50)  return { bg: "rgba(201,168,76,0.08)",  text: "#8a7040", label: "±1",   border: "rgba(201,168,76,0.15)" };
  if (score === 20)  return { bg: "rgba(255,255,255,0.05)", text: "#5a5040", label: "≤5",   border: "rgba(201,168,76,0.12)" };
  return { bg: "transparent", text: "#3a3020", label: "—", border: "transparent" };
}
...
function TeamLogo({ team, size = 32 }: { team: PickRow["teams"]; size?: number }) {
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: 6, background: "rgba(201,168,76,0.08)",
      display: "flex", alignItems: "center", justifyContent: "center",
      fontSize: size * 0.32, fontWeight: 700, color: "#6b5a30", flexShrink: 0 }}>
      {team?.abbr?.slice(0, 3) ?? "—"}
    </div>
  );
}
    ...
    return (
      <>
        {picks.map((p) => {
          ...
          let numColor = "#6b5a30";
          if (isActive) { numBg = "#c9a84c"; numColor = "#0c0d14"; }
          else if (filled && resultsReady) { numBg = ss.bg; numColor = ss.text; }
          else if (filled) { numBg = "rgba(201,168,76,0.12)"; numColor = "#c9a84c"; }
          ...
          return (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: filled ? "#f0ede4" : "#6b5a30",
                  whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                  {p.draft_players?.full_name ?? (isActive && isOwner ? "On the clock…" : "—")}
                </div>
                {p.draft_players && (
                  <div style={{ fontSize: 10, color: "#6b5a30", marginTop: 1 }}>
              ...
              {filled && isOwner && !picksLocked && (
                <button onClick={(e) => { e.stopPropagation(); clearPick(p.pick_no); }}
                  style={{ background: "none", border: "none", color: "#6b5a30", fontSize: 15,
    ...
    if (!isOwner) {
      return (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flex: 1,
          color: "#6b5a30", fontSize: 14, flexDirection: "column", gap: 8 }}>
      ...
      <>
        ...
        <div style={{ background: "rgba(201,168,76,0.04)", borderBottom: "1px solid rgba(201,168,76,0.1)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#c9a84c", letterSpacing: 2,
                textTransform: "uppercase", marginBottom: 4 }}>
                {currentPickRow?.draft_players ? "Ausgewählt" : "On the Clock"}
              </div>
              <div style={{ fontSize: 20, fontWeight: 800, color: "#f0ede4", lineHeight: 1.1 }}>
                {currentPickRow?.draft_players?.full_name ?? team?.name ?? "—"}
              </div>
              {currentPickRow?.draft_players ? (
                <div style={{ fontSize: 12, color: "#8a7a5a", marginTop: 3 }}>
                  ...
                  {" · Pick "}<span style={{ fontFamily: "monospace", fontWeight: 700, color: "#c9a84c" }}>{currentPick}</span>
                </div>
              ) : (
                <div style={{ fontSize: 12, color: "#8a7a5a", marginTop: 3 }}>
                  {team?.name} · Pick <span style={{ fontFamily: "monospace", fontWeight: 700, color: "#c9a84c" }}>{currentPick}</span>
                </div>
              )}
          ...
          {positions.map((p) => (
            <button key={p} onClick={() => setPos(p)}
              style={{ border: "1px solid", borderRadius: 20, padding: "4px 11px",
                ...
                color: pos === p ? "#0c0d14" : (p === "ALL" ? "#8a7a5a" : posColor(p)),
                borderColor: pos === p ? posColor(p === "ALL" ? "XX" : p) : "rgba(201,168,76,0.18)",
              }}>
              {p === "ALL" ? "Alle" : p}
            </button>
          ))}
        ...
        <div ref={playerListRef} style={{ overflowY: "auto", flex: 1 }}>
          {availablePlayers.length === 0 && (
            <div style={{ padding: "32px 20px", color: "#6b5a30", fontSize: 13, textAlign: "center" }}>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: "#f0ede4" }}>{p.full_name}</span>
                    {isNeed && (
                  ...
                  <div style={{ fontSize: 11, color: "#8a7a5a", marginTop: 2, display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontWeight: 700, color: pc,
                      background: `${pc}15`, borderRadius: 4, padding: "1px 5px", fontSize: 10 }}>
                      {p.position}
                    </span>
                    <span>{p.school}</span>
                    {p.rank_pos && <span style={{ color: "#6b5a30" }}>· {p.position}{p.rank_pos}</span>}
                  </div>
                </div>
                <div style={{ fontFamily: "monospace", fontSize: 12, fontWeight: 700, color: "#6b5a30", flexShrink: 0 }}>
                  #{p.rank_overall}
                </div>
        ...
        <div style={{ padding: "16px 14px 12px", borderBottom: "1px solid rgba(201,168,76,0.1)", flexShrink: 0 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 12, color: "#8a7a5a" }}>Fortschritt</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: "#f0ede4", fontFamily: "monospace" }}>
          ...
          {resultsReady ? (
            <div style={{ background: "rgba(201,168,76,0.08)", border: "1px solid rgba(201,168,76,0.25)",
              ...
              <div style={{ fontSize: 36, fontWeight: 800, color: "#c9a84c", lineHeight: 1, marginTop: 4, fontFamily: "monospace" }}>
                {totalScore}
              </div>
          ...
          {picks.map((p) => {
            ...
            return (
              ...
              >
                  ...
                  {p.draft_players && (
                    <div style={{ fontSize: 10, color: "#6b5a30", marginTop: 1 }}>
                      ...
                      {resultsReady && p.real_pick_no && (
                        <span style={{ marginLeft: 4, color: "#6b5a30" }}>· real #{p.real_pick_no}</span>
                      )}
                    </div>
                  )}
                </div>
                ...
                {filled && isOwner && !picksLocked && (
                  <button onClick={(e) => { e.stopPropagation(); clearPick(p.pick_no); }}
                    style={{ background: "none", border: "none", color: "#6b5a30", fontSize: 14,
    ...
    <>
      <style>{`
        ...
        .md-tab {
          ...
          color: #6b5a30; font-family: inherit;
          border-bottom: 2px solid transparent;
        }
        .md-tab.active { color: #c9a84c; border-bottom-color: #c9a84c; }
        .md-msg {
        ...
        .md-locked {
          margin: 8px 16px; background: rgba(201,168,76,0.08);
          border: 1px solid rgba(201,168,76,0.2); border-radius: 8px;
          padding: 8px 13px; font-size: 12px; color: #c9a84c;
        }
      ...
      <div className="md-root">
        ...
        <div className="md-topbar">
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <Link href="/mock-draft" style={{ fontSize: 13, color: "#6b5a30", textDecoration: "none",
              fontWeight: 600, transition: "color 0.15s" }}
              onMouseEnter={(e) => (e.currentTarget.style.color = "#f0ede4")}
              onMouseLeave={(e) => (e.currentTarget.style.color = "#6b5a30")}>
              ← Mock Draft
            </Link>
            <div style={{ width: 1, height: 18, background: "rgba(201,168,76,0.12)" }} />
            <div>
              <div style={{ fontSize: 14, fontWeight: 800, color: "#f0ede4" }}>{mock.title}</div>
              <div style={{ fontSize: 11, color: "#6b5a30" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ fontFamily: "monospace", fontSize: 12, color: "#6b5a30" }}>
            ...
            {resultsReady && totalScore !== null && (
              <div style={{ background: "rgba(201,168,76,0.12)", border: "1px solid rgba(201,168,76,0.3)",
                color: "#c9a84c", fontWeight: 800, fontSize: 13, borderRadius: 20, padding: "4px 12px",
