...
export default function PickemPage() {
  ...
  function renderScore(g: Game) {
    ...
    const final = (g.status ?? "").toUpperCase() === "FINAL" || g.winner_team_id != null;
    ...
    <main style={{ maxWidth: 1050, margin: "40px auto", fontFamily: "system-ui" }}>
      ...
      {!loading && standings.length > 0 && (
        <section style={{ marginTop: 16, border: "1px solid #eee", borderRadius: 12, padding: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <h2 style={{ margin: 0 }}>Standings (Season {SEASON_YEAR})</h2>
            <button
          ...
          <div style={{ overflowX: "auto", marginTop: 10 }}>
            <table style={{ borderCollapse: "collapse", width: "100%", minWidth: 700 }}>
              ...
              <tbody>
                {standings.map((r, idx) => (
                  <tr key={r.user_id} style={{ background: r.user_id === uid ? "#f7fbff" : "transparent" }}>
                    <td style={td}>{idx + 1}</td>
                    <td style={td}><b>{r.username}</b></td>
                    <td style={td}><b>{r.total_points}</b></td>
                    {standingsWeeks.map((w) => (
                      <td key={w} style={tdCenter}>{r.weekly?.[String(w)] ?? 0}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
